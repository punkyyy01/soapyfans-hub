  ...
  if (items.length === 0) {
    return (
      <p className="text-sm italic text-[var(--text-muted)]">
        No activity yet. The archive is waiting.
      </p>
    )
  }
    ...
    <li className="group flex gap-4 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]/50 p-4 transition-colors hover:border-[var(--accent-amber)]/30">
      ...
      <div className="flex-1 min-w-0 space-y-1.5">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
          ...
          {item.kind === 'music' && (
            <span className="rounded-full border border-[var(--accent-forest)]/60 px-2 py-px text-[0.55rem] uppercase tracking-[0.2em] text-[var(--accent-forest)]">
              Music
            </span>
          )}

          <span className="text-sm" aria-label={`${item.rating} out of 5 stars`}>
            <span className="text-[var(--accent-gold)]">{'★'.repeat(item.rating)}</span>
            <span className="text-[var(--text-muted)]">{'★'.repeat(5 - item.rating)}</span>
          </span>
    ...
    <form action={formAction} className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <p className="text-[0.65rem] uppercase tracking-[0.32em] text-[var(--accent-amber)]">
          Editing review
          <span className="ml-2 normal-case text-[var(--text-muted)]">— {item.title}</span>
        </p>
